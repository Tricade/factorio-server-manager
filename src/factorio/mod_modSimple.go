...
// SetModEnabled writes an explicit state for both downloaded and built-in mods.
// Factorio's expansion features live below data/ rather than in a ZIP archive,
// but they are enabled through the same mod-list.json file.
func (modSimpleList *ModSimpleList) SetModEnabled(modName string, enabled bool) error {
	return modSimpleList.SetModsEnabled(map[string]bool{modName: enabled})
}
...
func (modSimpleList *ModSimpleList) SetModsEnabled(states map[string]bool) error {
	...
	for index, mod := range modSimpleList.Mods {
		if enabled, ok := pending[mod.Name]; ok {
	...
	for _, name := range names {
		enabled := pending[name]
		modSimpleList.Mods = append(modSimpleList.Mods, ModSimple{Name: name, Enabled: enabled})
	}
	return modSimpleList.saveModInfoJson()
}

func (modSimpleList *ModSimpleList) IsEnabled(modName string) bool {
